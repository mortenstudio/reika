  ...
  return (
    <header className="absolute inset-x-0 top-0 z-20 pointer-events-none">
      <motion.div
        ...
        {...logoMotionProps}
      >
        <Link href="/" aria-label="Reika home" className="flex w-fit">
      ...
      <motion.div
        ...
        className="absolute top-32 md:top-4 left-0 md:left-1/2 right-4 mx-4 bg-white w-fit pointer-events-auto flex flex-col pb-3"
        {...motionProps}
      >
        {tagline ? (
          <div className="text-black text-md md:text-lg leading-tight -translate-y-1">
            {tagline}
          </div>
        ) : null}
        {subtagline ? (
      ...
      {navigationItems.length > 0 ? (
        <motion.nav
          key={`nav-${pathname}`}
          className="absolute top-4 right-4 bg-white w-fit pointer-events-auto flex flex-col lg:flex-row gap-1 items-end ml-3 pb-3"
          ...
          {navigationItems.map((page: NavigationPage) => {
            ...
            return (
              <Link
                key={page._id}
                href={href}
                className="bg-[#ECE7DA] text-black rounded-md px-3 py-2 cursor-pointer text-base text-right leading-tight decoration-black/25 w-fit hover:decoration-transparent hover:rounded-[20px] hover:bg-[#534129] hover:text-white transition-all duration-200"
              >
