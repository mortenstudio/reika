    ...
    if (viewMode === "list") {
      const row = (
        <div className="grid grid-cols-6 lg:grid-cols-12 xl:items-center bg-[#E5F1CC] rounded-md p-4 w-full">
          <h3 className="col-span-6 lg:col-span-2 text-md md:text-lg leading-tight mb-3 lg:mb-0">
            {model.name}
          </h3>
          <p className="col-span-6 lg:col-span-6 text-md md:text-lg leading-tight opacity-66">
            {model.description}
          </p>
          <div className="col-span-6 lg:col-span-4 lg:justify-self-end flex gap-1 mt-12 lg:mt-0">
            <div className="select-none text-xs font-mono uppercase bg-[#38422A] text-white text-center rounded-full px-2.5 py-1 flex gap-4">
    ...
    const gridInner = (
      <div className="flex flex-col gap-2 md:gap-4">
        ...
        <div className="bg-[#E5F1CC] rounded-md p-4 flex flex-col gap-3">
          <h3 className="text-md md:text-lg leading-tight">{model.name}</h3>
          <p className="text-md md:text-lg leading-tight lg:w-1/2 mb-12 md:mb-24 opacity-66">
            {model.description}
          </p>
          <div className="flex flex-col md:flex-row gap-1">
            <div className="select-none text-xs font-mono uppercase bg-[#38422A] text-white text-center rounded-full px-2.5 py-1 flex gap-4 w-fit">
              <span className="opacity-66">Størrelse</span><span>{model.size} kvm</span>
            </div>
            <div className="select-none text-xs font-mono uppercase bg-[#38422A] text-white text-center rounded-full px-2.5 py-1 flex gap-4 w-fit">
              <span className="opacity-66">Personer</span><span>{model.capacity}</span>
